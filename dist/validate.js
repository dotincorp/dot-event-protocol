...
import { CLOUD_EVENTS_SPEC_VERSION, DOT_EVENT_PROTOCOL_VERSION, DOT_EVENT_TYPE_PATTERN, ISO_COUNTRY_CODE_PATTERN, ISO_REGION_CODE_PATTERN, } from "./types.js";
 ...
 */
function collectUnknownKeys(record, allowed, path) {
...
const PRODUCER_KEYS = ["app", "appVersion", "platform", "deviceModel", "osVersion"];
const DATA_KEYS = [
    "actor",
    "action",
    "object",
...
export function validateDotEvent(value) {
    const issues = [];
    if (!isRecord(value)) {
    ...
    if (!isRecord(value.dot)) {
        ...
        if (!isRecord(value.dot.producer)) {
            ...
            for (const field of ["app", "appVersion", "platform"]) {
        ...
        if (value.dot.countryCode !== undefined &&
        ...
        if (value.dot.regionCode !== undefined &&
        ...
        else if (typeof value.dot.regionCode === "string" &&
...
export function assertDotEvent(value) {
